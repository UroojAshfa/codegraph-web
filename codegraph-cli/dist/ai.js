...
class AIService {
    ...
    async explainFunction(functionName, functionCode, complexity, callers, callees) {
...
Keep it brief and actionable. Focus on what matters.`;
    ...
    async analyzeComplexity(functionName, complexity, lineCount) {
...
Respond in 2-3 sentences. Be direct and actionable.`;
    ...
    async suggestRefactoring(functionName, functionCode, complexity) {
        if (complexity < 8) {
...
Keep it practical and specific to this code.`;
    ...
    async detectCodeSmells(functionCode, complexity, lineCount, paramCount) {
        const smells = [];
